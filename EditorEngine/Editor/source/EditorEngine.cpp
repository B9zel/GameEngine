...
EditorEngine::EditorEngine(const InitializeObject& Initilize) : Engine(Initilize)
{
	...
	/*	CoreEngine::Render::FramebufferSpecification Spec;
		...
		EFramebufferTextureFilterConfig::LINER));

		FrameBuffer = CoreEngine::Render::Framebuffer::Create(Spec);*/

	// Create UI
	Viewport = MakeSharedPtr<Editor::EditorViewport>();
	Viewport->SetOwnerEditor(this);
	SceneHier = MakeSharedPtr<Editor::SceneHierarhy>();
	SceneHier->SetOwnerEditor(this);

	DetailsPanel = MakeSharedPtr<Editor::EditorDetails>();
	DetailsPanel->SetOwnerEditor(this);

	MenuBar = MakeSharedPtr<Editor::EditorMenuBar>();
...
Editor::EditorViewportClient* EditorEngine::GetViewpoertClient() const
{
